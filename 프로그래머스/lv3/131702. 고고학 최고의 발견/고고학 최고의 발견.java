...
class Solution {
    ...
    public int solution(int[][] clockHands) {
        ...
        // 4^(n*n) 경우의 수가 있다.
        // 조건을 통해 경우의 수를 줄여야한다.
        // - 회전 후 인접한 위(=상)의 값이 0 이어야 한다.
        // - minCount보다 이미 count가 크다면 skip한다.
    ...
    public void recursive(int[][] clocks, int count, int depth){
        ...
        if(isComplete(clocks)){minCount = Math.min(minCount, count); return;}
        if(depth >= n*n) return;
    ...
    public boolean checkUpValue(int[][] clocks, int r, int c){
        int nr = r + dir[0][0];
        int nc = c + dir[0][1];
        if(!valid(nr, nc)) return true;
        if(clocks[nr][nc] == 0) return true;
        return false;
    }
    ...
    public boolean isComplete(int[][] clocks){
        for(int i = 0; i < n; i++) {
            if(!Arrays.equals(clocks[i], zero)) return false;
        }
        return true;
    }
}